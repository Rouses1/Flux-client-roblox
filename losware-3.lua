...
-- Банихоп: прыгаем каждый тик земли + velocity манипуляция
local bhopActive = false
local function startBhop()
	if bhopConn then bhopConn:Disconnect(); bhopConn = nil end
	bhopActive = true
	bhopConn = RunService.Heartbeat:Connect(function()
		if not S.BunnyHop or not bhopActive then return end
		local hum = getHum()
		local hrp = getHRP()
		if not hum or not hrp then return end
		if hum.FloorMaterial ~= Enum.Material.Air then
			-- Прыгаем сразу как касаемся земли
			hum.Jump = true
			-- Сохраняем горизонтальный импульс
...
-- =============================================
-- ANTI-AIM: дёргаем CFrame HRP чтобы сбить
-- чужой аимбот, без влияния на наш ввод
-- =============================================
...
local function startAntiAim()
	...
	antiAimConn = RunService.RenderStepped:Connect(function()
		...
		antiAimOffset = antiAimOffset + 1
		-- Быстрое вращение HRP по Y — сбивает hitbox-scan у других
		-- Делаем это только визуально на клиенте через CFrame
		local yaw = math.rad(antiAimFlip * 180 * (antiAimOffset % 2))
		if S.AntiAimPitch then
			-- Дополнительный pitch jitter
			local pitch = math.rad(math.sin(tick() * 20) * 89)
			hrp.CFrame = hrp.CFrame * CFrame.Angles(pitch, yaw, 0)
		else
			hrp.CFrame = hrp.CFrame * CFrame.Angles(0, yaw, 0)
		end
		antiAimFlip = -antiAimFlip
	end)
end
	...
	while isFarming do
		...
		if not target then
			...
			if reached and target.Parent and afOreReachable(target) then
				...
				afMine(target)
				-- Помечаем как добытую
				minedOres[oreId] = tick()
				currentOreTarget = nil
			else
				-- Не смогли добраться — помечаем на короткое время
				if target.Parent then
					minedOres[tostring(target)] = tick() - 5
				end
			end
			task.wait(0.1)
		end
...
local TABS = {"aimbot", "visuals", "misc", "autofarm", "movement", "friends"}
local tabBtns = {}
local tabFrames = {}
local activeTab = nil
...
do
	...
	addSec(t, "bunny hop", 4)
	addToggle(t, "Bunny Hop", "BunnyHop", 5, function(v)
		if v then startBhop() else stopBhop() end
	end)
	addSec(t, "anti-aim", 6)
	addToggle(t, "Anti-Aim (Yaw)", "AntiAim", 7, function(v)
		if v then startAntiAim() else stopAntiAim() end
	end)
	addToggle(t, "Pitch Jitter", "AntiAimPitch", 8)
end
